...
class Spring {
  ...
  constructor(parameters = {}) {
    ...
    this.restThreshold = .0006; // Values below this threshold are considered resting position
    this.restDuration = 100; // Duration in ms used to check if the spring is resting after reaching restThreshold
...
class Draggable {
  ...
  constructor(target, parameters = {}) {
    ...
    this.hasCustomEase = !!customEase && !hasSpring;
    ...
    this.updateTicker = new Timer({ autoplay: false, onUpdate: self => this.update(self) }, null, 0).init();
    this.manual = false;
    this.grabbed = false;
    this.dragged = false;
    this.updated = false;
    ...
    // Make sure the draggable is init inside the draggable area
    const [bt, br, bb, bl] = this.containerBounds;
    this.x = clamp(this.x, bl, br);
    this.y = clamp(this.x, bt, bb);
  }
  ...
  updateBoundingValues() {
    ...
    this.canScroll = fx ? false :
      ...
      (!this.container || (this.container && !isArr(this.container))) &&
      this.contained;
    if (this.contained) {
  ...
  update(timer) {
    ...
    const x = this.x;
    const y = this.y;
    this.coords[0] += dpx * this.dragSpeed;
    this.coords[1] += dpy * this.dragSpeed;
    this.pointer[2] = px;
    this.pointer[3] = py;
    ...
    let newY = 0;
    if (!nx) {
      newX = this.x = clamp(cx, cl, cr) + (cx - clamp(cx, cl, cr)) * cf;
    }
    if (!ny) {
      newY = this.y = clamp(cy, ct, cb) + (cy - clamp(cy, ct, cb)) * cf;
    }
    ...
    const dx = newX - x;
    const dy = newY - y;
    const updateXVelocity = !nx && dx && (dx < 0 ? newX > cl : newY < cr);
    const updateYVelocity = !ny && dy && (dy < 0 ? newY > ct : newY < cb);
    const previousVelocity = this.velocity;
    let v = 0;
    if (updateXVelocity || updateYVelocity) {
      const dt = timer.deltaTime;
      v = (dt > 0 ? Math.sqrt(dx * dx + dy * dy) / dt : 0) * this.releaseVelocity;
  ...
  handleDown(e) {
    ...
    // remove(this); // Removes any active animations on this draggable
    ...
    this.x = cx;
    this.y = cy;
    this.coords[0] = this.coords[2] = clamp(cx, cl, cr) + (cx - clamp(cx, cl, cr)) / cf;
    this.coords[1] = this.coords[3] = clamp(cy, ct, cb) + (cy - clamp(cy, ct, cb)) / cf;
    this.pointer[2] = x;
    this.pointer[3] = y;
    this.deltaX = 0;
    this.deltaY = 0;
  ...
  handleUp() {
    ...
    const hasCustomEase = this.hasCustomEase;
    const cx = this.x;
    const cy = this.y;
    const rv = this.velocity * globals.timeScale; // Velocity needs to be adjusted to be properly calculated when using seconds as timeUnit
    const ds = rv * 100;
    const dx = Math.cos(this.angle) * ds;
    const dy = Math.sin(this.angle) * ds;
    const x = cx + dx;
    const y = cy + dy;
    const cf = 1 - this.containerFriction;
    spring.velocity = round(clamp(ds / 50, 0, 50), 2);
    const { ease, duration } = spring;
    let destX = clamp(snap(x, this.snapX), bl, br);
    let destY = clamp(snap(y, this.snapY), bt, bb);
    if (this.isOutOfBounds(this.containerBounds, x, y) && cf) {
      // Apply a spring easing if the target ends outside the container
      ...
      // const bouncedX = destX;
      // const bouncedY = destY;
      const easeX = by || hasCustomEase ? releaseEase : ease;
      const easeY = bx || hasCustomEase ? releaseEase : ease;
      ...
      // Use a simple ease if the target ends inside the container
      // Duration should be 0 if destination value is the same as current value
      // Duration should be 0 if velocity is disabled
      const durX = cx !== destX || ds ? duration : 0;
      const durY = cy !== destY || ds ? duration : 0;
      if (!disabledX) this.animate[this.xProp](destX, durX, releaseEase);
      if (!disabledY) this.animate[this.yProp](destY, durY, releaseEase);
    }
    ...
    this.scrollInView(this.scrollThreshold, duration, releaseEase);
    this.onRelease(this);
...
if (isBrowser) {
  ...
  win.AnimeJS.push({ version: '4.0.0-beta.102', engine });
